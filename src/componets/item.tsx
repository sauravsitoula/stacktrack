...
export default function Item({ itemData, userLevel, context }: ItemParams) {
  ...
  async function deleteItem() {
    const response = await fetch(
      "http://18.118.122.21:3000/api/items/" + itemData?.uuid,
      {
        method: "DELETE",
        headers: {
    ...
    <Card raised sx={{ padding: 2 }}>
      <Box textAlign="center">
        {context === "single" ? (
          <Image
            src={
              itemData?.image_url?.startsWith("http")
                ? itemData?.image_url
                : Placeholder
            }
            alt={itemData?.name || ""}
      ...
      <Typography sx={{ textAlign: "center" }}>{itemData?.name}</Typography>
      ...
      {context === "list" || userLevel === "regular" ? (
        itemData?.quantity || 0 > 0 ? (
          ...
          <Typography
            sx={{ color: "red", textAlign: "center", fontSize: "25px" }}
          >
            Out of Stock
          </Typography>
        )
